...
Delta for the Cognee dual-brain change. Adds a switchable brain mode, a graph-aware Cognee
recall lane beside the existing similarity lanes, a Cognee graph endpoint mirroring the vault
graph contract, and the deployment of a fully-local Cognee service — all under the standing
guarantee that the vault is the source of truth and the Cognee lane is fail-open.

(No established `agent-memory` spec exists yet, so every requirement below is ADDED.)
