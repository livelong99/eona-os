...
def init_agent(
    ...
    if api_mode in {"chat_completions", "codex_responses", "anthropic_messages", "bedrock_converse", "codex_app_server", "claude_code"}:
        # "claude_code" delegates the turn to the local `claude` CLI runtime
        # (see agent/conversation_loop.py and agent/claude_code_runtime.py).
        # It must survive init verbatim so the loop can detect it; without this
        # the value would fall through to the "chat_completions" default below.
