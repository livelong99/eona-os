...
def register_dashboard_routes(app: "Any", adapter: "Any") -> None:
    ...
    # Board routes (/v1/tasks, /v1/events, /v1/approvals) — extracted into the
    # dashboard.board_routes module (behaviour unchanged).
    from gateway.platforms.dashboard import board_routes
    board_routes.register(app, adapter)
    # Memory galaxy routes (/v1/memory, /graph, /note, /search) — extracted into
    # the dashboard.memory_routes module (behaviour unchanged).
    ...
    # Control screen (/v1/usage, /v1/model-config) + Planner integrations
    # (/v1/integrations/jira, /v1/integrations/gmail) — extracted into the
    # dashboard.control_routes module (behaviour unchanged).
    from gateway.platforms.dashboard import control_routes
    control_routes.register(app, adapter)
    ...
    # Literal workspace routes (create/browse/rename/exec/exec-stop) — registered
    # before the generic {tool_id} routes so they aren't shadowed. Extracted into
    # the dashboard.workspace_routes module (behaviour unchanged).
    from gateway.platforms.dashboard import workspace_routes
    workspace_routes.register(app, adapter)
