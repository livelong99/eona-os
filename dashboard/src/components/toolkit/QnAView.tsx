...
import type { QnAQuestion } from "@/lib/brainstorm/brainstormClient";
...
export function QnAView({ questions, drafts, onDraftChange, onSubmit, processing }: QnAViewProps) {
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto px-5 py-4">
        {open.map((q) => (
          <div key={q.id} className="rounded-xl border border-white/12 bg-white/[0.03] p-4">
            <QuestionMeta agent={q.agent} category={q.category} />
            <p className="mt-2 text-[14px] font-medium leading-snug text-white/90">{q.question}</p>
            {q.why && <p className="mt-1.5 text-[11.5px] italic leading-relaxed text-white/40">{q.why}</p>}
            <textarea
              value={drafts[q.id] ?? ""}
        ...
        {answered.length > 0 && (
          <>
            ...
            {answered.map((q) => (
              <div key={q.id} className="rounded-xl border border-white/[0.06] bg-white/[0.015] p-4 opacity-80">
                <QuestionMeta agent={q.agent} category={q.category} resolved />
                <p className="mt-2 text-[13.5px] font-medium leading-snug text-white/70">{q.question}</p>
                <p className="mt-2 rounded-lg bg-white/[0.04] px-3 py-2 text-[13px] leading-relaxed text-white/55">{q.answer}</p>
              </div>
            ))}
          </>
        )}
...
function QuestionMeta({ agent, category, resolved }: { agent?: string; category?: string; resolved?: boolean }) {
  return (
    <div className="flex items-center gap-2 text-[11px]">
      {agent && <span className="font-medium text-white/60">{agent}</span>}
      {category && <span className="rounded-md bg-white/[0.06] px-1.5 py-0.5 text-white/50">{category}</span>}
      {resolved && (
